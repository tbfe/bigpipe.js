(function () {
    ...
    ResourceManager.prototype.load = function (ids) {
        ...
        if (ids.length > 0) {
            this.resourceLoader.load(this.resourcesComboed);
        } else {
            return Promise.resolve();
        }
    };

    ResourceManager.prototype._loadResource = function (type) {
        var loadResources = [];
        var createLoadResource = function (url) {
            var resource = ResourceManager.resourcePool[url];
            var loadResource = new Promise(function (resolve) {
                if (resource) {
                    resource.count++;
                    resource.loader.then(resolve);
                } else {
                    if (type === ResourceManager.RESOURCE_TYPE_SCRIPT) {
                        Utils.loadScript(url).then(resolve);
                    } else {
                        Utils.loadStyle(url).then(resolve);
                    }
                }
            });

            ResourceManager.resourcePool[url] = {
    ...
    ResourceManager.prototype.registerResources = function (urls) {
        var self = this;
        var resourcesLocal = self.resourcesLocal;
        ...
        Utils.each(resourcesLocal, function (url, i) {
            ...
            self.resourceLoader._comboMap[url] = resourceCombedTemp[part];
        });
    };
    ...
    Pagelet.prototype = {
        ...
        _loadStyle: function () {
            this.styleResourceManager.load(this.styles);
        },
        _loadScript: function () {
            this.scriptResourceManager.load(this.scripts);
        },
