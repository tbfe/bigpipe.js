(function() {
    ...
    resourceManager.combo = function (ids) {
        var resources = resourceManager._resources;
        var resourcesComboed = resourceManager._resourcesCombed;
        var urls = [
            '/??'
        ];
        var urlsCombed = '';
        for (var i = 0; i < ids.length; i++ ) {
            ...
            if (resources[url]) {
                ...
                urls.push((urls.length > 1 ? ',' : '') + url);
            }
        }
        if (urls.length > 1) {
            urlsCombed = urls.join('');
            resourcesComboed.push(resourcesComboed);
            for (var i = 0; i < ids.length; i++ ) {
                var url = ids[i];
                resources[url] = urlsCombed;
            }
        }
        ...
        var createLoadStyle = function(url) {
            ...
            var loadStyle = new Promise(function(resolve) {
                if (style) {
                    ...
                    // styleManager 这里需要拿到 stylesheet 的引用
    ...
    function ResourceManager (id, scripts, styles) {
        var self;
